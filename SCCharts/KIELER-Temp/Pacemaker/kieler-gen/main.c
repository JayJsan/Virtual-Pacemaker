...
void receiveVariables() {
    ...
    if(root != NULL) {
        ...
        if(item != NULL) {
            model.VEvents_Clock = item->valuedouble;
        }
        // Receive _GO
...
void sendVariables(int send_interface) {
    ...
    // Send AR
    cJSON_AddItemToObject(root, "AR", cJSON_CreateBool(model.AR));
    // Send VP
    cJSON_AddItemToObject(root, "VP", cJSON_CreateBool(model.VP));
    // Send VR
    cJSON_AddItemToObject(root, "VR", cJSON_CreateBool(model.VR));
    // Send sleepT
    ...
    // Send VEvents_Clock
    cJSON_AddItemToObject(root, "VEvents_Clock", cJSON_CreateNumber(model.VEvents_Clock));
    // Send _GO
    ...
    cJSON_AddItemToObject(root, "_region0_Main_AVI_Stop", cJSON_CreateBool(model._region0_Main_AVI_Stop));
    // Send _region0_Main_VEvents_Stop
    cJSON_AddItemToObject(root, "_region0_Main_VEvents_Stop", cJSON_CreateBool(model._region0_Main_VEvents_Stop));
